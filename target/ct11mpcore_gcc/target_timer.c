 ...
 *  の責任を負わない．
 * 
 *  $Id: target_timer.c 56 2015-08-23 02:25:49Z ertl-hiro $
 */
...
{
	sil_wrw_mem(SP804_CR(TWDTIMER_TIMER_BASE), SP804_DISABLE);
	sil_wrw_mem(SP804_ICR(TWDTIMER_TIMER_BASE), 0U);
...
{
	if ((sil_rew_mem(SP804_MIS(TWDTIMER_TIMER_BASE)) & 0x01U) != 0U) {
		sil_wrw_mem(SP804_ICR(TWDTIMER_TIMER_BASE), 0U);
		twd_switch();				/* タイムウィンドウ切換え処理 */
	}

#ifdef TOPPERS_SUPPORT_OVRHDR
	if ((sil_rew_mem(SP804_MIS(OVRTIMER_TIMER_BASE)) & 0x01U) != 0U) {
		sil_wrw_mem(SP804_ICR(OVRTIMER_TIMER_BASE), 0U);
